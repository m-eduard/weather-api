import os

import db_utils
from flask import Blueprint, Response, current_app, jsonify, request
from pymongo import MongoClient
from validators import (
    Operations,
    dependencies,
    unique_fields,
    validation_error,
    validators,
)

with current_app.app_context():
    mongo_client = MongoClient(current_app.config["mongo_uri"])
    db = mongo_client[os.environ.get("DB_NAME")]
    collection = db.get_collection("temperatures")

    api_temperatures = Blueprint("api_temperatures", __name__)

    @api_temperatures.route("/api/temperatures/", methods=["POST"])
    def post_temperature():
        body = request.get_json(silent=False)
        if not validators[Operations.POST_TEMPERATURE](body):
            return Response(validation_error(Operations.POST_TEMPERATURE), status=400)

        try:
            new_temperature = db_utils.insert_if_unique(
                collection,
                body,
                unique_fields[Operations.POST_TEMPERATURE],
                dependencies[Operations.POST_TEMPERATURE],
                timestamp=True,
            )

            # Throw an error if a city with the same name and country id already exists
            if not new_temperature:
                raise Exception(
                    f"Another temperature with the same {unique_fields[Operations.POST_TEMPERATURE]} already exists in {collection.name}"
                )
        except Exception as e:
            return Response(
                str(
                    {
                        "operation": Operations.POST_TEMPERATURE.value,
                        "collection": collection.name,
                        "body": body,
                        "error": e,
                    }
                ),
                status=409,
            )

        response = jsonify({"id": new_temperature.inserted_id})
        response.status_code = 201

        return response
